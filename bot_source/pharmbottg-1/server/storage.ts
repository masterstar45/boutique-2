...
export class DatabaseStorage implements IStorage {
  ...
  async addToCart(item: InsertCartItem): Promise<CartItem> {
    // Check if item with same product AND same price/weight option exists
    ...
    // Find item with matching price/weight
    const existing = existingItems.find(e => 
      e.selectedPrice === item.selectedPrice && 
      e.selectedWeight === item.selectedWeight
    );
  ...
  async upsertLoyaltyBalance(chatId: string, points: number, tier: string, totalEarned: number): Promise<LoyaltyBalance> {
    ...
    if (existing) {
      const [updated] = await db.update(loyaltyBalances)
        .set({ points, tier, totalEarned })
        .where(eq(loyaltyBalances.chatId, chatId))
        .returning();
      return updated;
    }
    const [newBalance] = await db.insert(loyaltyBalances)
      .values({ chatId, points, tier, totalEarned })
      .returning();
    return newBalance;
  }
  ...
  async addLoyaltyPoints(chatId: string, delta: number, reason: string, orderCode?: string, description?: string): Promise<LoyaltyBalance> {
    ...
    let newTier = 'bronze';
    if (newTotalEarned >= settings.goldThreshold) {
      newTier = 'gold';
    } else if (newTotalEarned >= settings.silverThreshold) {
      newTier = 'silver';
    }
  ...
  async getLoyaltyTransactions(chatId: string, limit: number = 20): Promise<LoyaltyTransaction[]> {
    return await db.select()
      ...
      .where(eq(loyaltyTransactions.chatId, chatId))
  ...
  async getTopLoyaltyUsers(limit: number = 10): Promise<LoyaltyBalance[]> {
    return await db.select()
      ...
      .orderBy(desc(loyaltyBalances.totalEarned))
      .limit(limit);
  }
  ...
  async searchLoyaltyUser(query: string): Promise<LoyaltyBalance | undefined> {
    const [balance] = await db.select()
      .from(loyaltyBalances)
      .where(eq(loyaltyBalances.chatId, query));
    return balance;
  }
  ...
  async validateAccessPassword(password: string): Promise<AccessPassword | null> {
    const activePasswords = await db.select().from(accessPasswords)
      .where(eq(accessPasswords.active, true));
    
    for (const pwd of activePasswords) {
      const isMatch = await bcrypt.compare(password, pwd.password);
      if (isMatch) {
        if (pwd.usageLimit && pwd.usageCount >= pwd.usageLimit) {
          return null;
        }
        
        await db.update(accessPasswords)
          .set({ usageCount: pwd.usageCount + 1 })
          .where(eq(accessPasswords.id, pwd.id));
  ...
  async unlockUser(chatId: string): Promise<void> {
    ...
    if (existingUser) {
      await db.update(botUsers)
        .set({ isUnlocked: true, unlockedAt: new Date().toISOString() })
  ...
  async getFavorites(chatId: string): Promise<FavoriteWithProduct[]> {
    ...
    return items.map(({ favorite, product }) => ({
      ...favorite,
      product,
    }));
  }
  ...
  async addFavorite(chatId: string, productId: number): Promise<Favorite> {
    const [existing] = await db.select().from(favorites)
      .where(and(eq(favorites.chatId, chatId), eq(favorites.productId, productId)));
    if (existing) return existing;
    
    const [newFav] = await db.insert(favorites)
      .values({ chatId, productId })
      .returning();
    return newFav;
  }
  ...
  async getSavedAddresses(chatId: string): Promise<SavedAddress[]> {
    return await db.select().from(savedAddresses)
      .where(eq(savedAddresses.chatId, chatId));
  }

  async addSavedAddress(address: InsertSavedAddress): Promise<SavedAddress> {
    const [newAddress] = await db.insert(savedAddresses)
      .values(address)
      .returning();
    return newAddress;
  }
  ...
  async getClientButtons(activeOnly: boolean = false): Promise<ClientButton[]> {
    if (activeOnly) {
      return await db.select().from(clientButtons)
        .where(eq(clientButtons.active, true))
        .orderBy(clientButtons.position);
    }
  ...
  async updateClientButton(id: number, button: Partial<InsertClientButton>): Promise<ClientButton | undefined> {
    const [updated] = await db.update(clientButtons)
      .set(button)
      .where(eq(clientButtons.id, id))
      .returning();
    return updated;
